...
> 5. **Jest en Windows**: correr por DIRECTORIO (`npx jest src/lib src/app`), NUNCA `--testPathPattern` (cuelga). Y los patterns posicionales son **regex**: `(auth)` sin escapar NO matchea y los tests **no corren** pareciendo verdes.
