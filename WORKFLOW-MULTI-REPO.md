...
- **SIEMPRE worktree, NUNCA sobre `main` directo (INNEGOCIABLE).** Todo trabajo de **código** (feature, fix, refactor) se hace en un **worktree dedicado por cambio**, jamás editando/implementando sobre el working tree de `main`. El `main` local queda limpio y solo avanza por merge/pull. *(Única excepción: los docs de proceso — este archivo, `BACKLOG.md`, `DEUDAS-PENDIENTES.md` — se editan directo en `main`.)*
  ...
  - **BORRAR un worktree (cleanup) — MÉTODO SEGURO OBLIGATORIO.** Cazado el 2026-06-24: un `git worktree remove --force` con el junction `node_modules` PRESENTE RECORRIÓ el junction y VACIÓ el `node_modules` REAL del BE (485 paq → 0, síntoma "Filename too long" durante el remove). `git worktree remove` **también** sigue el junction, igual que `rm -rf` — NUNCA lo corras con el junction presente. Procedimiento por worktree: **(1)** quitar el junction primero con .NET (NO sigue al target): `powershell -NoProfile -Command "$nm = Join-Path '<win_path>' 'node_modules'; if (Test-Path -LiteralPath $nm) { (Get-Item -LiteralPath $nm -Force).Delete() }"`; **(2) CONFIRMAR** que se quitó antes de remover: `if [ -e "$wt/node_modules" ]; then echo SKIP; continue; fi` (si el Delete falló, el SKIP protege el `node_modules` real); **(3)** recién entonces `git -C <repo> worktree remove --force <wt>` + `git -C <repo> branch -D <branch>`. **Verificar el real después de cada borrado:** `ls <repo>/node_modules/.bin`. **Recuperación si se rompió:** `npm ci` (reinstala desde el lock — NO es build). `cmd //c "if exist X rmdir X"` NO fue confiable para quitar el junction; usar el `.Delete()` de .NET. Antes de borrar cualquier worktree: `git -C <wt> status --short` para no enterrar trabajo sin commitear (y NUNCA tocar worktrees `agent-*` con cambios pendientes o `locked` — son de sesiones/agentes activos).
