...
>
> 3. **Espejo navegable en Obsidian (es VISTA, NO fuente — decision del usuario 2026-06-26).** Existe un vault Obsidian en `C:\Users\ronald\Documents\IPNext\Backlog\` (LOCAL, NO versionado) con una nota por card + `_Backlog (MOC).md` + `Ideas.md`, interconectado con `[[links]]` al resto del vault (`[[RADIUS]]`, `[[Arquitectura]]`, `[[Cutover RDA2]]`, ...). Sirve para que el operador NAVEGUE el grafo. **La FUENTE DE VERDAD sigue siendo `BACKLOG.md` en el repo** (git): es lo que leen el workflow y los sub-agentes — el vault local NO lo ven. Por eso: las cards se crean/actualizan SIEMPRE en `BACKLOG.md`; el vault Obsidian se **REGENERA desde el BACKLOG on-demand** (cuando el operador quiere navegar el grafo al dia), NO es obligatorio re-sincronizarlo en cada card. NUNCA borrar el `BACKLOG.md` para "migrar a Obsidian": romperia la trazabilidad compartida/versionada que ven agentes y sesiones paralelas.
>
