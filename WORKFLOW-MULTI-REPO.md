...
Los cuatro viven uno al lado del otro bajo `C:\Users\ronald\projects\ipnext\`.
...
**BE / FE / orchestrator** tienen `.github/workflows/deploy.yml` con un runner **self-hosted**. Hacer `git push` a `main` **auto-deploya a PRODUCCIÓN**. No hay staging. *(Excepción: el **NOC Collector (Rust)** todavía NO — su `deploy` es un stub y el deploy es manual; ver la nota de arriba.)*
