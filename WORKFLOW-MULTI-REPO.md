...
| **Customer App (mobile)** | `C:\Users\ronald\projects\ipnext\ipnext-customer-app` | `github.com/rhernandezbas/ipnext-customer-app` (**PRIVADO**) | **Expo + React Native + TypeScript** (SDK 57, RN 0.86, expo-router, TS strict). App de **CLIENTES finales** del ISP: saldos, facturas, planes, tickets, tareas. Android primero, iOS después. Creado el **2026-07-29** desde CERO (la app mobile anterior quedó legacy y se descarta). Consume el **Backend** por REST (superficie `/api/portal/*`, a construir — JWT audience de cliente separado del admin, queries ancladas al `customerId` del token). |
