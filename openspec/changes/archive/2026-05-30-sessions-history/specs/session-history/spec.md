...
**Type**: Delta — agrega `GET /api/admin/sessions/history` y extiende `SessionRepository`.
**Change**: `sessions-history`
**Routes touched**: `GET /api/admin/sessions/history` (nuevo). `GET /api/admin/sessions` sin modificaciones.
...
### REQ-SREPO-1: SessionRepository.findRevoked (MODIFIED — extensión aditiva)
