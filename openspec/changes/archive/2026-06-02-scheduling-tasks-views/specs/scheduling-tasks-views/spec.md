...
### REQ-FILTER-7: No filter params returns all tasks (backward compat)

**Given** an authenticated `GET /api/scheduling` with no query params
**Then** the server MUST respond exactly as before this change — HTTP 200 with the full task list
