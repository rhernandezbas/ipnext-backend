...
remainingToday}}`. `valid[].variables` MUST ser el mapa MERGEADO efectivo de ese recipient (no el
global crudo) — es lo que el humano audita antes de autorizar el `send`.
...
(VAL-6/VAL-7), y opt-out no cambió desde el `validate` — un preview válido puede rechazarse acá si
el estado cambió.
