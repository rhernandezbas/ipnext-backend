...
**Source change**: `gestion-real-installation-ingest` (archived 2026-05-30)

---

## Purpose

Provide a typed, editable config store for the ingest engine plus read endpoints the "Gestión Real"
FE subpage consumes. Config controls how often the ingest runs and which local `Project` each
technology maps to. All endpoints return DTOs, never raw Prisma entities. Use cases depend only on
ports.
...
needs-review `ScheduledTask` DTOs — the tasks the ingest created with no project assigned.

The implemented predicate is `grOrdenId IS NOT NULL AND projectId IS NULL`: it returns exactly the
ingest-produced tasks that still need a technology/project decision. This is equivalent in practice
to "ingest task with `projectId = null` (and `[REVISAR - Logística]` title prefix)" — a normal
fiber/wireless task always has a non-null project and is therefore excluded — and is preferred over
title-string matching, which is fragile/i18n-brittle.

#### Scenario: Returns only needs-review tasks

- GIVEN one needs-review task (ingest task, no project) and one normal fiber task (with project)
