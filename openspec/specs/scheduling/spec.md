...
Adds support for network-mode tasks linked to `NetworkSite` entities instead of customers, enabling third-party infrastructure monitoring and maintenance tasks.
...
(Previous: no `kind`, `networkSiteId`, or `networkSiteName` fields; no `stageId`/`stageCategory`/`reviewedByInventory`)
...
### Requirement: REQ-VAL-1 — `CreateTaskSchema` is a discriminated union on `kind` (MODIFIED)
...
## Appendix: New Error Codes (network-node-task)
