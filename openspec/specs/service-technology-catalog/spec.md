...
**Capability**: `service-technology-catalog` (NEW)
**Change**: `service-technology`
**Summary**: An editable CRUD catalog of service technology types (Fiber, DOCSIS, Wireless, etc.) with unique name and optional description. Mirrors `TaskCategory` exactly. `Service.technology` stores the technology name as a free string; the catalog drives the UI dropdown. No FK relation in Phase 1.

---

## Overview

The catalog is an admin-managed list of technology types. Name uniqueness is enforced case-insensitively at the use-case layer (stored as-provided). A delete guard prevents removal of a technology that is referenced by any `Service.technology` value. The `GET /api/services` endpoint provides a global paginated listing of all service contracts with optional filters.

---

## 1. Service Technology Catalog (CRUD)
...
## 2. Service Technology Assignment
...
## 3. Services Listing (Global Contracts)
...
| Scenario | HTTP | `code` |
|----------|------|--------|
| Not found | 404 | `SERVICE_TECHNOLOGY_NOT_FOUND` |
| Name conflict (create/update) | 409 | `SERVICE_TECHNOLOGY_NAME_CONFLICT` |
| Technology in use (delete) | 409 | `SERVICE_TECHNOLOGY_IN_USE` |
