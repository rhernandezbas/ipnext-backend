...
describe('ListServices use case', () => {
  ...
  let listServices: ListServices;

  beforeEach(() => {
    repo = new InMemoryServiceRepository();
    listServices = new ListServices(repo);
  });

  it('returns an empty page envelope when there are no services', async () => {
    const res = await listServices.execute({ page: 1, limit: 25 });
  ...
  it('filters by status (exact match)', async () => {
    ...
    const res = await listServices.execute({ page: 1, limit: 25, status: 'blocked' });
    expect(res.total).toBe(1);
  ...
  it('filters by technology (exact match)', async () => {
    ...
    const res = await listServices.execute({ page: 1, limit: 25, technology: 'Wireless' });
    expect(res.total).toBe(1);
  ...
  it('search matches plan (case-insensitive)', async () => {
    ...
    const res = await listServices.execute({ page: 1, limit: 25, search: 'fiber' });
    expect(res.total).toBe(1);
  ...
  it('search matches client name (case-insensitive)', async () => {
    ...
    const res = await listServices.execute({ page: 1, limit: 25, search: 'juan' });
    expect(res.total).toBe(1);
  ...
  it('paginates and computes totalPages', async () => {
    ...
    const res = await listServices.execute({ page: 2, limit: 2 });
  ...
  it('defaults page to 1 and limit to 25 when omitted', async () => {
    ...
    const res = await listServices.execute({});
