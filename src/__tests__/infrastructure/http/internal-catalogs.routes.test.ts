...
describe('GET /api/internal/catalogs/teams — no auth required', () => {
  it('defaults to active=true — inactive/cancelled teams are NOT returned (#134)', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.items).toHaveLength(1);
    expect(res.body.items[0]).toMatchObject({ login: 'equipe-1', name: 'Equipe Uno', active: true, selectable: true });
  });

  it('?active=false returns only inactive/cancelled teams', async () => {
    const { app, teamRepo } = buildApp();
    await teamRepo.upsertByLogin({ login: 'equipe-1', name: 'Equipe Uno', thirdPartyCode: null, active: true, selectable: true });
    await teamRepo.upsertByLogin({ login: 'equipe-2', name: 'Equipe Dos', thirdPartyCode: 'TP-2', active: false, selectable: true });

    const res = await request(app).get('/api/internal/catalogs/teams').query({ active: 'false' });

    expect(res.status).toBe(200);
    expect(res.body.items).toHaveLength(1);
    expect(res.body.items[0].login).toBe('equipe-2');
  });

  it('?active=true is equivalent to the default', async () => {
    const { app, teamRepo } = buildApp();
    await teamRepo.upsertByLogin({ login: 'equipe-1', name: 'Equipe Uno', thirdPartyCode: null, active: true, selectable: true });
    await teamRepo.upsertByLogin({ login: 'equipe-2', name: 'Equipe Dos', thirdPartyCode: 'TP-2', active: false, selectable: true });

    const res = await request(app).get('/api/internal/catalogs/teams').query({ active: 'true' });

    expect(res.status).toBe(200);
    expect(res.body.items).toHaveLength(1);
    expect(res.body.items[0].login).toBe('equipe-1');
  });
});
