 ...
 * resolved, mentioned, snoozed }`. Siete `ConversationRepository.count` disparados EN PARALELO
 * (`Promise.all` — un solo roundtrip lógico, siete `COUNT` triviales sobre
 ...
 * conversation-snooze (Ola 6c) — `snoozed` = POSPUESTAS VIGENTES (status='snoozed' AND
 * snoozedUntil>now); mismo builder del where que `?view=snoozed` (el badge no diverge de la vista).
 *
...
export class GetInboxViewCounts {
  ...
  async execute(userId: string): Promise<InboxViewCountsDto> {
    ...
    const [all, mine, unassigned, unattended, resolved, mentioned, snoozed] = await Promise.all([
      ...
      // Mismo builder del where que el listado `?view=snoozed`: el badge no diverge de la vista.
      this.conversationRepo.count({ snoozed: true }),
    ]);

    return { mine, unattended, all, unassigned, resolved, mentioned, snoozed };
  }
}
