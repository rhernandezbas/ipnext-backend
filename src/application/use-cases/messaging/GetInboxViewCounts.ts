 ...
 * resolved }`. Cinco `ConversationRepository.count` disparados EN PARALELO
 * (`Promise.all` — un solo roundtrip lógico, cinco `COUNT` triviales sobre
...
export class GetInboxViewCounts {
  ...
  async execute(userId: string): Promise<InboxViewCountsDto> {
    ...
    return { mine, unattended, all, unassigned, resolved };
  }
}
