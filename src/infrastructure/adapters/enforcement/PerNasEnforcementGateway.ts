 ...
 *
 *   nas.type === 'radius_orchestrator'  → RADIUS (orchestrator + CoA)   ← NAS ya cutoveado al HA
 ...
 * local (solo Acceso Sur tiene RADIUS). Un NAS pasa a cortar por RADIUS marcándose
 * `radius_orchestrator` — sin big-bang, NAS por NAS. Cualquier `type` desconocido cae a MK-directo (el
