...
export class InMemoryNewsPostRepository implements NewsPostRepository {
   ...
   * Prisma resolves this via `include: { category: true }`; the in-memory adapter
   * mirrors that by resolving against an injected NewsCategoryRepository (defaults
   * to a private, empty one for isolated unit tests that don't care about the
  ...
  async findById(id: string, userId: string): Promise<NewsPostWithReadState | null> {
    ...
    if (!item) return null;
    return { ...item, read: this.receipts.has(this.key(id, userId)) };
  }
  ...
  async list(filters: NewsPostListFilters, userId: string): Promise<NewsPostWithReadState[]> {
    ...
    const withRead: NewsPostWithReadState[] = result.map((i) => ({
      ...i,
