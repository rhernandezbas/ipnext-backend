 ...
 */
export function parseOcrResponse(text: string): { sn: string | null; mac: string | null } {
  const match = text.match(/\{[\s\S]*\}/);
  if (!match) return { sn: null, mac: null };
  try {
    const obj = JSON.parse(match[0]) as Record<string, unknown>;
    return { sn: norm(obj.sn), mac: norm(obj.mac) };
  } catch {
    return { sn: null, mac: null };
  }
}
