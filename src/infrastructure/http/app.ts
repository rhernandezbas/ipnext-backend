...
// portal-usage-metrics — "Mi consumo" de Mis servicios. Lee del accounting de
// RADIUS (`RadiusEvent`) las sesiones que SOLAPAN el mes en UNA query; el
// contrato se une a sus usernames PPPoE por `PppoeService.contractId`, el
// camino que el modelo ya tiene. El prorrateo vive en el use case.
...
export function createApp(taskAutocomplete?: TaskAutocompleteScheduler | null, backfillScheduler?: BackfillScheduler | null, uispSyncScheduler?: UispSyncScheduler | null, financeReceiptIngestScheduler?: FinanceReceiptIngestScheduler | null) {
  ...
  // Phase F task F.4, design D3/D7) — the unified bot-action audit repo +
  // the 4 write ports (`reply` is wired separately as a plain HTTP adapter,
  // Phase G; `note`/`status`/`close` are REAL when the sidecar envs are
  // set, `Unavailable*Port` fallbacks otherwise, molde
  // `UnavailableSuricataReplyPort`'s conservative-guard pattern).
  ...
  const closeSuricataTicket = new CloseSuricataTicket(
    suricataTicketRepo,
    suricataBotActionAuditRepo,
  ...
  // PPPoE para resolver contrato -> usernames de RADIUS (todos: vigente +
  // históricos). El reader es propio y de SOLO LECTURA: trae de `RadiusEvent`
  // las sesiones solapantes del mes en una única query.
